...
- new: Python sends average room temperatures to Arduino
...
async def Two_Room_Thermo_Sync():
    ...
    try:
        ...
        setpoint_rm1 = 80.0
        setpoint_rm2 = 80.0
        gains_rm1 = (8.0, 0.03, 0.0) #Kp, Ki, Kd
        gains_rm2 = (8.0, 0.03, 0.0)
        ...
        with open(state_filename, 'w', newline='') as f:
            csv.writer(f).writerow(["Time (h)", "Heater 1", "Heater 2", "Output 1 (%)", "Output 2 (%)"])
        ...
        while (time.time() - start_time) < total_time:
            ...
            if current_time - last_save_time >= 900:
                ...
                with open(filename, 'a', newline='') as f:
                    w = csv.writer(f)
                    for k in new_idx:
                        row = [f"{timestamps[k]:.2f}"] + [f"{temperatures[ch][k]:.2f}" if k < len(temperatures[ch]) else "" for ch in channels]
                        w.writerow(row)

                with open(state_filename, 'a', newline='') as f:
                    w = csv.writer(f)
                    for k in new_idx:
                        w.writerow([f"{timestamps[k]:.2f}", heater1_state[k], heater2_state[k], f"{heater1_output_hist[k]:.1f}", f"{heater2_output_hist[k]:.1f}"])
